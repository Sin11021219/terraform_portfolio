...
### 【工夫した点】
・パブリックサブネットへのALB・EC2配置と、プライベートサブネットによるRDSの隔離(NAT Gatewayのコストを削減するためEC2をパブリックサブネットへ配置し、EC2のインバウンドSSHを完全に閉じ、Session Managerによる制御とALBからの通信のみにSGを絞り込むことで、パブリックサブネットにありながらも実質的にプライベートと同等の安全性を担保する設計とした点。)
